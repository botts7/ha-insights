...
| v1.15 | **HardwareSuggestionDetector** — fill data gaps | Meta-detector that runs AFTER existing detectors. When a detector fires weakly because the install lacks the right sensor class (lights stay on >3 h in area with no motion sensor → suggest motion / mmWave / BLE presence), emit `InsightKind.HARDWARE_SUGGESTION` with the *category* of device that would close the gap. `lib/hardware_gap_rules.py` (composable pure-function rules: one per gap type — presence, contact, energy, per-room temp, AQ, outdoor, water, illuminance). **Hard non-commercial guardrails: categories only, no brands/SKUs, no buy buttons, no affiliate links, no price guidance, per-suggestion dismiss + per-area cooldown.** Gated on user having set `goals_json` so we only suggest to users who opted-in to proactive advice. |
| v1.16 | **AdaptiveFeedbackDetector** — learn from accept/reject/retire | Meta detector consuming per-detector verdict history + environmental-change signals. Emits two kinds: (1) "you've rejected 18/20 X-detector suggestions, disable it?" (2) "we retired this 6 months ago but the automation it referenced was deleted / the sensor was moved — reconsider?" New `lib/user_verdict_history.py` (per-fingerprint accept/reject/retire counts, rolling window) + `lib/environmental_diff.py` (per-fingerprint context-changed-since-retire detection). **Never auto-disables a detector or unretires silently** — always surfaces as an actionable insight. |
| v2.0 | **Per-person presence + identity-aware insights** | New mental model: every detector that could be person-aware gets a `person_id` dimension. `lib/person_location.py` (per-person device_trackers + BLE proximity → per-(person, area, time) location probability) + `lib/identity_disambiguation.py` (negative-constraint reasoning: "can't be Alice if she's not home"). Publishes `sensor.ha_insights_<person>_location`. PersonalSchedulePatternDetector emits "Alice usually arrives 17:30 ± 12 min on weekdays" (not just "someone arrives"). Per-insight "That was X, not me" feedback corrects identity weights over time. Privacy: per-person opt-out, redactor extends to per-person IDs in LLM context. **Prerequisites: HA `person.*` entities + companion app + recommended BLE proxies / mmWave.** |
| v2.x+ | Polish + research backlog | HAWatcher shadow execution (97 % precision validation); 2nd-order Markov + Shannon-entropy AIT approximation; survival-aware reminder cards |
